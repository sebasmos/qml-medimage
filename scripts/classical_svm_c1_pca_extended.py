#!/usr/bin/env python3
"""
Classical SVM C=1 baselines for missing PCA dimensions across all 3 models.

Missing configs:
  - MedSigLIP-448: PCA-6, 8, 16
  - RAD-DINO:      PCA-10, 16
  - ViT-patch32:   PCA-10, 16

Reproduces the EXACT preprocessing from the QSVM pipeline (DT9):
  1. 80/10/10 stratified split with random_state=seed
  2. StandardScaler (fit on train)
  3. PCA(n_components=q) (fit on train)
  4. MinMaxScaler([-1,1]) (fit on BOTH train+test combined — legacy behavior)
  5. SVM with C=1.0, kernel={linear, rbf}

Output: tests/c1_forced_extended/svm/{model}/data_type9/pca_{q}/metrics_summary.csv
"""
...
OUTPUT_BASE = os.path.join(PROJECT_ROOT, "tests", "c1_forced_extended", "svm")

MODELS = {
    "medsiglip-448": {
        "data_path": "/orcd/pool/006/lceli_shared/DATASET/qml-mimic-cxr-embeddings/medsiglip-448-embeddings/20-seeds/seed_0/data_type9_n2371.parquet",
        "pca_dims": [6, 8, 16],
        "data_type_dir": "data_type9",
    },
    "rad-dino": {
        "data_path": "/orcd/pool/006/lceli_shared/DATASET/qml-mimic-cxr-embeddings/rad-dino-embeddings/20-seeds/seed_0/data_type9_n2370_seed0_rad_dino.parquet",
        "pca_dims": [10, 16],
        "data_type_dir": "data_type9",
    },
    "vit-patch32-cls": {
        "data_path": "/orcd/pool/006/lceli_shared/DATASET/qml-mimic-cxr-embeddings/vit-base-patch32-224-embeddings/20-seeds/seed_0/data_type9_n2371_cls_embedding.parquet",
        "pca_dims": [10, 16],
        "data_type_dir": "data_type9",
    },
}
...
def main():
    print("=" * 80)
    print("CLASSICAL SVM C=1 BASELINE — EXTENDED (missing PCA dims)")
    print("=" * 80)
    print(f"Seed: {SEED}")
    print(f"C: {C_VALUE}")
    ...
    for model_name, cfg in MODELS.items():
        ...
        print(f"PCA dims to run: {cfg['pca_dims']}")
        print(f"{'='*80}")

        X, y, class_names = load_and_prepare(cfg["data_path"])
        X_train, X_val, X_test, y_train, y_val, y_test = split_data(X, y)

        model_output = os.path.join(OUTPUT_BASE, model_name, cfg["data_type_dir"])

        for q in cfg["pca_dims"]:
            print(f"\n--- {model_name} / PCA-{q} ---")
            ...
            out_dir = os.path.join(model_output, f"pca_{q}")
    ...
    # Print grand summary
    print("\n" + "=" * 80)
    print("GRAND SUMMARY: Classical SVM C=1")
    print("=" * 80)
    print(f"{'Model':<20} {'PCA':>4} {'Kernel':>8} {'Test Acc':>10} {'Test F1':>10} {'Test AUC':>10}")
    print("-" * 70)
    for r in grand_results:
        print(f"{r['model']:<20} {r['pca_dim']:>4} {r['kernel']:>8} "
              f"{r['test_accuracy']:>10.4f} {r['test_f1']:>10.4f} {r['test_auc']:>10.4f}")

    # Save combined
    combined_path = os.path.join(OUTPUT_BASE, "extended_all_models_summary.csv")
    pd.DataFrame(grand_results).to_csv(combined_path, index=False)
